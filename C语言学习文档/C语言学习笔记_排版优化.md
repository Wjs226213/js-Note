...
>**思路**
>
>	当前代码的执行逻辑是首先我要寻找一个数字作为最大并且将这个数字排序到最后面，其次我每当排序成功一次我都会少排序一轮。
>**外层循环**
>
>	外层循环控制的是当前的需要找几次最大的数值，是长度少一次因为不要和自己完成比较
>**内层**
>
>	内层循环控制的是当前轮要和后面的数据完成排序，首先当前数字是要和下一个数字完成比较，所以在最后一个数字+1的时候会出现错误，那么就进行长度-1可以防止索引越界，同时后面还有以恶搞-1的操作i的数值会随着轮数的变化而变换，我经过前面的轮数排序我后面就不需要将排序后的结果动它所以每次执行一轮排序数量就少一个从0开始因为开始的时候没有任何的变换到1-2-3
