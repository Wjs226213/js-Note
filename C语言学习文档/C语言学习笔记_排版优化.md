...
### 2级指针
*指针变量指向的是其他数据的内存地址但是同时指针变量同时也会有一个字节的内存地址。*

---
**补充**
* 在前面的知识点中说明了指针变量保存的是变量的内存地址可以通过内存地址访问原始变量的数据，从而可以根据指针变量中的内存地址访问和修改数据。

* 2级别的指针保存的是1级别指针的内存地址，1级指针的内存地址中保存的是变量的内存地址，所以通过2级别指针的可以访问1级别指针的内存地址从而可以去改变指针变量中保存的数据也是就是变量的数据。
---

**总结**
指针变量是用来保存变量的内存地址的，可以访问和修改变量的数据，
2级指针使用用来保存指针变量的内存地址的可以修改和保存指针变量数据
